...
const Masonry: React.FC<MasonryProps> = ({
  ...
  const { grid, gridHeight } = useMemo(() => {
    ...
    return { grid: positioned, gridHeight: Math.max(...colHeights, 0) };
  }, [columns, items, width]);
  ...
  return (
    <div ref={containerRef} className="relative w-full" style={{ height: gridHeight || 0 }}>
      {grid.map(item => (
        <div
          key={item.id}
          data-key={item.id}
        ...
        >
          <div
            className="relative w-full h-full rounded-[12px] shadow-[0px_10px_40px_-12px_rgba(0,0,0,0.25)] overflow-hidden"
            style={{
              backgroundImage: item.img ? `url(${item.img})` : undefined,
              background: item.background || (item.img ? undefined : 'linear-gradient(135deg, #111, #222)'),
              backgroundSize: 'cover',
              backgroundPosition: 'center',
            }}
          >
            {item.label && (
              <div className="absolute inset-0 bg-black/30 flex items-end p-4 text-white text-sm font-semibold tracking-wide">
                {item.label}
              </div>
            )}
            {colorShiftOnHover && <div className="color-overlay absolute inset-0 rounded-[12px] bg-gradient-to-tr from-pink-500/50 to-sky-500/50 opacity-0 pointer-events-none" />}
          </div>
        </div>
      ))}
    </div>
