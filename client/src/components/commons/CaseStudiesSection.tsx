...
import { getPublishedCaseStudies, type CaseStudy } from '../../services/caseStudiesApi';
import { caseStudiesSectionConfig } from './caseStudiesData';
  ...
  useEffect(() => {
    if (!propCaseStudies) {
      const fetchCaseStudies = async () => {
        try {
          const response = await getPublishedCaseStudies();
          setFetchedCaseStudies(response.data || []);
        } catch (error) {
          console.error('Failed to fetch case studies:', error);
          setFetchedCaseStudies([]);
        } finally {
          setLoading(false);
        }
      };
