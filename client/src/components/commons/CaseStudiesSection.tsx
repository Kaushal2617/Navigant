...
const CaseStudiesSection: React.FC<CaseStudiesSectionProps> = ({
  ...
  useEffect(() => {
    // Only fetch if no props provided (or if we want to mix them, but here we prioritize props)
    if (!propCaseStudies) {
      const fetchCaseStudies = async () => {
        try {
          ...
          if (response.ok) {
            ...
            // Map backend response to frontend CaseStudy interface
            const mappedData: CaseStudy[] = data.map((item: any) => ({
              ...
              readMoreLink: `/case-studies/${item.id}`,
              alt: item.alt || item.title,
              fullContent: item.fullContent
            }));
            setFetchedCaseStudies(mappedData);
          } else {
