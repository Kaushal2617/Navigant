...
public class LeadServiceImpl implements LeadService {
	...
	public Page<LeadResponse> getAllLeads(Pageable pageable) {
		...
		// Step 1: Get page of leads (1 query)
		Page<Lead> leadPage = leadRepository.findAll(pageable);
		List<Lead> leads = leadPage.getContent();

		// Step 2: Collect unique admin IDs who reviewed leads (only for this page)
		Set<String> adminIds = leads.stream()
				.map(Lead::getReviewedBy)
				.filter(Objects::nonNull)
				.filter(id -> !id.isEmpty())
				.collect(Collectors.toSet());

		// Step 3: Batch fetch all admins in ONE query
		Map<String, String> adminNames = adminRepository.findAllById(adminIds)
				.stream()
				.collect(Collectors.toMap(Admin::getId, Admin::getName));

		// Step 4: Map leads to responses using the cached admin names, preserving Page
		// metadata
		return leadPage.map(lead -> {
			String reviewerName = lead.getReviewedBy() != null
					? adminNames.get(lead.getReviewedBy())
					: null;
	...
	@Override
	public void exportLeadsToCsv(java.io.Writer writer) {
		try (org.apache.commons.csv.CSVPrinter printer = new org.apache.commons.csv.CSVPrinter(writer,
				org.apache.commons.csv.CSVFormat.DEFAULT.builder()
						.setHeader("ID", "Name", "Email", "Phone", "Service", "Seats", "Status", "Date")
						.build())) {
